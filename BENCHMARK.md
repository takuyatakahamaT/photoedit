...
2026-09-24に、manifest v4を現行PhotoCore契約と校正機Mac Studioへ再固定した（現行のmanifest SHA-256は`CALIBRATION.md`を参照）。本書のrunは、旧manifest（`9da1fd58…`）とMac miniに固定した2026-07-24の履歴で、再固定後のbenchmarkは未実行である。benchmark runnerも同じmanifestを読むため、今後はMac Studioでしか実行できない。性能は実行機に依存するので、Studioで測った値をMac miniの7月の値と直接比べない。
