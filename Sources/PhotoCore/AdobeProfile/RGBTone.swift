...
// The 1025-point default tone-curve table embedded in `ACR3DefaultToneCurve`
// below is transcribed verbatim from Adobe DNG SDK 1.7.1's
// `dng_render.cpp`, `dng_tone_curve_acr3_default::Evaluate`'s `kTable`.
// License terms: see `.photobench/engine-research-20260922/LICENSE.txt`.

public enum RGBToneError: Error, Sendable, Equatable, LocalizedError {
    case tooFewSplinePoints
    case splineKnotsNotStrictlyIncreasing

    public var errorDescription: String? {
        switch self {
        case .tooFewSplinePoints: "スプライン制御点が2点未満です。"
        case .splineKnotsNotStrictlyIncreasing: "スプライン制御点のxは厳密な単調増加が必要です。"
        }
    }
}

/// Hue-preserving RGB tone-curve application, `dng_reference.cpp`'s
/// `RefBaselineRGBTone`. Also hosts the ACR3 default curve
/// (`dng_tone_curve_acr3_default`) and the DNG point-curve spline solver
/// (`dng_spline.cpp` / `dng_spline.h`), both of which are only ever used
/// *through* this hue-preserving application in the phase1 pipeline.
public enum RGBTone {
    /// Applies a [0,1]->[0,1] tone curve to an RGB triple while preserving
    /// its hue: sorts (r,g,b), evaluates `curve` at the max and min, and
    /// places the middle value at the same fractional position between the
    /// new min/max as it held between the old min/max. Inputs are pinned to
    /// [0,1] first, exactly as the reference does unconditionally
    /// (independent of HDR/overrange support).
    ///
    /// This mirrors the SDK's literal 7-way case analysis (see
    /// `RefBaselineRGBTone`'s `RGBTone(...)` macro invocations) rather than
    /// a generic sort, so the degenerate `r == g == b` case (the only one
    ...
    public static func apply(_ rgb: SIMD3<Double>, curve: (Double) -> Double) -> SIMD3<Double> {
        ...
        let b = min(max(rgb.z, 0.0), 1.0)

        // Returns (hiOut, midOut, loOut) = (curve(hi), interpolated, curve(lo)).
        func mix(hi: Double, mid: Double, lo: Double) -> (hi: Double, mid: Double, lo: Double) {
            let hiOut = curve(hi)
            let loOut = curve(lo)
            let denom = hi - lo
            let midOut = denom > 0 ? loOut + (hiOut - loOut) * (mid - lo) / denom : hiOut
            return (hiOut, midOut, loOut)
        }

        if r >= g {
            if g > b {
                let m = mix(hi: r, mid: g, lo: b) // Case 1: r >= g > b
                return SIMD3(m.hi, m.mid, m.lo)
            } else if b > r {
                let m = mix(hi: b, mid: r, lo: g) // Case 2: b > r >= g
                return SIMD3(m.mid, m.lo, m.hi)
            } else if b > g {
                let m = mix(hi: r, mid: b, lo: g) // Case 3: r >= b > g
                return SIMD3(m.hi, m.lo, m.mid)
            } else {
                // Case 4: r >= g == b (the only case hi could equal lo).
                let rr = curve(r)
                let gg = curve(g)
                return SIMD3(rr, gg, gg)
            }
        } else {
            if r >= b {
                let m = mix(hi: g, mid: r, lo: b) // Case 5: g > r >= b
                return SIMD3(m.mid, m.hi, m.lo)
            } else if b > g {
                let m = mix(hi: b, mid: g, lo: r) // Case 6: b > g > r
                return SIMD3(m.lo, m.mid, m.hi)
            } else {
                let m = mix(hi: g, mid: b, lo: r) // Case 7: g >= b > r
                return SIMD3(m.lo, m.hi, m.mid)
            }
        }
    }
}

/// `dng_tone_curve_acr3_default`: the fixed 1025-point default ACR3 tone
/// curve, table-lookup with linear interpolation.
public enum ACR3DefaultToneCurve {
    /// `dng_render.cpp`'s `kTable`, transcribed verbatim (1025 values).
    public static let table: [Double] = [
    ...
    /// `dng_tone_curve_acr3_default::Evaluate`: table lookup with linear
    /// interpolation over `x` clipped to [0,1].
    public static func evaluate(_ x: Double) -> Double {
        let clipped = min(max(x, 0.0), 1.0)
        let y = clipped * Double(table.count - 1)
        let index = min(max(Int(y.rounded(.down)), 0), table.count - 2)
        let frac = y - Double(index)
        return table[index] * (1.0 - frac) + table[index + 1] * frac
    }
}

/// Direct port of `dng_spline_solver` (`dng_spline.cpp` / `dng_spline.h`):
/// a not-a-knot-like C0/C1/C2-continuous cubic spline through control
/// points, used for Adobe Color's `ToneCurvePV2012` point curve. Flat
/// (constant) extension beyond the endpoints, matching the SDK's default
/// `fUseSlopeExtensionLo/Hi == false` (no slope-extension option is exposed
/// here since nothing in phase1 needs it).
public struct DNGSpline: Sendable {
    private let x: [Double]
    private let y: [Double]
    private let slopes: [Double]

    /// `points` need not be pre-sorted; they are sorted by x here (matching
    /// the Python prototype's `DngSplineSolver.__init__`, which the DNG SDK
    /// itself does not do -- the SDK's `dng_spline_solver::Add` assumes
    /// call-order-is-x-order and only validates strict monotonicity in
    /// `Solve()`). Throws if fewer than 2 points remain after sorting, or if
    /// any two points share an x value (SDK: `DNG_REQUIRE(X[k] > X[k-1])`).
    public init(points: [(x: Double, y: Double)]) throws {
        let sorted = points.sorted { $0.x < $1.x }
        guard sorted.count >= 2 else { throw RGBToneError.tooFewSplinePoints }
        let xs = sorted.map(\.x)
        let ys = sorted.map(\.y)
        for k in 1..<xs.count {
            guard xs[k] > xs[k - 1] else { throw RGBToneError.splineKnotsNotStrictlyIncreasing }
        }
        self.x = xs
        self.y = ys
        self.slopes = Self.solve(x: xs, y: ys)
    }

    /// `dng_spline_solver::Solve`.
    private static func solve(x: [Double], y: [Double]) -> [Double] {
        let count = x.count
        ...
        for j in 1..<count {
            let aj = 1.0 - f[j - 1] * e[j]
            if j != count - 1 { f[j] /= aj }
            g[j] = (g[j] - g[j - 1] * e[j]) / aj
        }

        var j2 = count - 2
        while j2 >= 0 {
            g[j2] = g[j2] - f[j2] * g[j2 + 1]
            j2 -= 1
        }
    ...
    /// `EvaluateSplineSegment` (`dng_spline.h`).
    private static func evaluateSegment(
        _ xVal: Double, x0: Double, y0: Double, s0: Double, x1: Double, y1: Double, s1: Double
    ) -> Double {
        let a = x1 - x0
        let b = (xVal - x0) / a
        let c = (x1 - xVal) / a
    ...
    /// `dng_spline_solver::Evaluate`. Flat extension beyond the endpoints.
    public func evaluate(_ xVal: Double) -> Double {
        let count = x.count
        if xVal <= x[0] { return y[0] }
        if xVal >= x[count - 1] { return y[count - 1] }

        var lower = 1
        var upper = count - 1
        while upper > lower {
            let mid = (lower + upper) >> 1
            if xVal == x[mid] { return y[mid] }
            if xVal > x[mid] {
                lower = mid + 1
            } else {
                upper = mid
        ...
        let j = lower
        return Self.evaluateSegment(
            xVal, x0: x[j - 1], y0: y[j - 1], s0: slopes[j - 1], x1: x[j], y1: y[j], s1: slopes[j]
        )
    }
}
