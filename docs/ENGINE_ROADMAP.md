...
**実エンジンでの振幅係数の格子探索（2026-09-23、Studio、RAW=pre-tone / 非RAW=s-p1-p2 固定）**: kH × kS の 16 組（320 描画）。学習＝H/S 単体 18 ＋ tone-all 3 ＋ 非RAW round0 3、ホールドアウト＝full 3 ＋ 4 プリセット × 2 scene。

| kH | kS | 学習 ΔE / EV | ホールドアウト ΔE / EV |
|---:|---:|---:|---:|
| 1.0 | 1.0（位置変更のみ） | 3.27 / +0.04 | 4.06 / −0.16 |
| 0.65 | 0.6（学習最良） | **2.36** / −0.03 | 4.12 / −0.19 |
| 0.5 | 1.0（ホールドアウト最良） | 3.32 / +0.10 | **3.36** / −0.06 |
| **0.5** | **0.8（採用）** | 2.68 / +0.04 | 3.41 / −0.11 |

- 学習は kS を下げるほど改善（単体 S+ が強すぎる scene が 2/3）、ホールドアウトは kS=1.0 が最良で逆方向。両者から僅差の **kH=0.5、kS=0.8** を採用。真の解は scene 依存（画像適応）で、round2 セット A で法則化する。
...
- 実装（`206880a`、v1）: 写真ごとに中立レンダーを長辺 750px に縮小して統計量を 1 回計算し、kS を決める（`SpatialGainScale.adaptive`）。再計測: H/S 単体 18 ケース 2.73 → 1.85、セット A 18 ケース 3.37 → 2.28。一方 tone-all 2.70 → 3.01、full 2.82 → 3.43、RAW プリセットは微悪化、JPEG プリセットは改善（8 組平均 3.53 → 3.45）。
- **16 scene での再フィット**: 法則はほぼ不変 `kS = clamp(0.4008 + 1.6900·highlightRatioBase, 0.4, 1.8)`（LOO −28%、複合 variant −22%）。kH は p95 で −2.9% の弱い効果のみ（見送り）。
- **統計量を取る画像の検証**: 中立 → 露出のみ → 露出＋コントラスト＋白／黒 適用後（preHS）の順に合成ケースが改善し、「H/S が効く時点の画像で測る」方向は正しい。ただし合成 10 ケース中 8 ケースは固定 kS=0.8 の方がまだ良く、単体データだけの fit では H×S×露出の相互作用を捉えきれない。**採用（v2、`spatial-adaptive` 定数 16 scene 版、統計量は H/S 適用直前の画像）**: A/B（A=適応 v2、B=固定 0.5/0.8）で 6 群中 5 群が A 優位。

| ケース群 | n | A（採用） | B（固定） |
|---|---:|---:|---:|
| H/S 単体 | 18 | **1.82** | 2.73 |
| tone-all_bluesky2 | 3 | **2.66** | 2.70 |
| full_bluesky2 | 3 | **2.58** | 2.83 |
| 非RAW round0 | 4 | 1.97 | 1.99 |
| 4 プリセット × 2 scene | 8 | **3.25** | 3.52 |
| round2 セット A（16 scene × 3） | 48 | **4.51** | 6.02 |
...
- 現行の非RAW 既定（RAW の適応則を流用）は単体で固定 kS 0.8 にも負ける（LOO 2.08 vs 1.89）。**非RAW 専用の法則が必要**。
- 採用: `kH = 0.7`（固定）、`kS = clamp(0.620 + 1.915·highlightRatio(入力), 0.6, 2.2)`、`sShift = clamp(0.236 + 0.652·p90(入力), −2, 0)`、位置は露出前（`s-p1-p2`）のまま。統計量は**入力 JPEG**から（非RAW は空間処理が最初なので「H/S が効く時点の画像」＝入力）。
