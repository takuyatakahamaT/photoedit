...
- **統計量を取る画像の検証**: 中立 → 露出のみ → 露出＋コントラスト＋白／黒 適用後（preHS）の順に合成ケースが改善し、「H/S が効く時点の画像で測る」方向は正しい。ただし合成 10 ケース中 8 ケースは固定 kS=0.8 の方がまだ良く、単体データだけの fit では H×S×露出の相互作用を捉えきれない。次: 統計量を preHS 画像で取る実装に変え、固定 0.8 と同じ表で比較して既定を決める。届かなければ (a) 単体寄りのときだけ適応、(b) 予測値と 0.8 の按分、(c) 合成を直接 fit する family、のいずれか。
