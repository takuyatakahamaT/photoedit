...
24 sourceを固定したtagged baselineのrelease benchmarkを直列に3回実行すると、process-fresh previewは`960.77 / 369.03 / 575.27 ms`で3 / 3合格、warm slider proxyは`396.32 / 59.84 / 52.75 ms`で0 / 3合格、high-quality previewと原寸JPEGは3 / 3合格だった。[Google Benchmarkの反復・warm-up指針](https://google.github.io/benchmark/user_guide.html)を参考に、全runと40 workerのsampleを保持し、遅い値をoutlierとして削除・再試行していない。schema v3は1 / 5 / 15分load averageも残すが、[`getloadavg`](https://developer.apple.com/library/archive/documentation/System/Conceptual/ManPages_iPhoneOS/man3/getloadavg.3.html)はrun queueの時間平均であり、瞬間的なGPU実行・同期・readbackを直接測るものではない。開始1分loadは`1.144 / 1.669 / 1.067`で、各workloadの変動をload average単独では説明できなかった。現行branchのmanifest契約は26 sourceへ更新済みであり、校正とbenchmarkのformal rerunはまだ実施していない。

画質不採用の縮小decodeを前提にせず、[Core Image render destinationの公式例](https://developer.apple.com/documentation/coreimage/generating-an-animation-with-a-core-image-render-destination)に沿ってproductionのfull-decode graphをMetal-backed destinationへ直接描画するopt-in経路を実装した。presentation lifecycleは純粋なreducerへ切り出し、request / latest queue、surface visibility、上限付き再試行、可視時間10秒deadline、timeout / teardownを決定的に扱う。GPU完了とpresented callbackの実行順は前提にせず、lock保護したsubmission arbiterが両方を観測し、GPU errorを優先しながらdeadline / callback競合をexactly-onceで解決する。Appleが示すとおり[`MTKView.currentDrawable`](https://developer.apple.com/documentation/metalkit/mtkview/currentdrawable)はnilになり得るため、再試行・可視性・一方向fallbackを維持する。

LaunchServicesから通常のforeground `.app`として起動した実機smokeでは、on-demandの`metal-clear`、`ci-solid`、`production`でpositive presentationへ到達した。各起動の初回callbackは[`presentedTime`](https://developer.apple.com/documentation/metal/mtldrawable/presentedtime)が0だったが、上限付き再試行で正値を得た。`presentedTime`は絶対時刻でありinput-to-present latencyではない。productionでは写真選択・調整更新、window resize、約12秒のminimize後のresumeでも最新requestを提示できた。一方、actual-screen parity、input-to-present p95、drop率、rapid supersede時の一過性stale-frame、複数写真切替後の定常RSSは未承認である。このためdirect経路はopt-in、既定はlegacyとする。

次の性能研究候補には、操作中の暫定表示と停止後の高品質settleを分ける案、zoomed-out表示とは別に100% detail windowを持つ案がある。[darktableのpixelpipe](https://docs.darktable.org/usermanual/development/en/darkroom/pixelpipe/the-pixelpipe-and-module-order/)は対話中に処理量を減らすpipeを、[RawTherapeeのdetail window](https://rawpedia.rawtherapee.com/Editor)は縮小表示で省く重い処理を小さな100%領域で確認する設計を示す。Photo Benchではこれらをそのまま移植せず、zoomed-out応答、高品質settle、100% detail、原寸exportを別々に受け入れ判定する。Metal直接表示はpositive presentationとlifecycle reducerまで成立したため、追加調査はactual-screen parity、input-to-present p95、drop率、rapid supersede時のstale-frame、定常RSSの受入計測に限定し、合格しない限り既定legacyを維持する。
...
現行の自動回帰はSwift Testing `130 tests / 10 suites`とPython `52 tests`である。curve端点外挿と重複x規則、OKLChの低彩度保護、HSL/curve段別CPU fixture、generic RAWのY>1合成出力経路、C1 shoulder、固定L/h gamut compression、67,368点の運用／stress色域grid、bounded-sRGB neutral bypass、DC-S5限定EDR1 profile、preview / full-resolution intentと、ΔE / EV / clip / plateau / preview parityのゲートを含む。加えて、canonical final 2,560px、3,072 / 3,840px候補、共通Lanczos、morphology、候補選択と原寸fallback、native寸法の0・非有限・不明と非有限extentの整数化前拒否、preview / export context isolation、system-load snapshot、benchmarkのpass / performanceFailed / notEvaluated、path traversal / symlink / alias / 欠測をfail-closedで検証する。Metal直接表示ではaspect fit、direct / legacy native rasterの1 LSB parity、latest-only queueのexpected-ID・resize・再入・順序逆転に加え、純粋なlifecycle reducerを24件、GPU / presented callback arbiterを6件、presentation probeを4件、diagnostic rendererを3件で検証する。reducerはsurface visibility、再試行、可視時間deadline、timeout、teardownを、arbiterはcallback順序、GPU error優先、deadline / invalidate後の抑止とexactly-onceを担う。ただしAppKit / WindowServerを通したactual-screen parity、drop率、rapid supersede時のstale-frame、定常RSSは自動回帰では保証していない。

正式校正・benchmarkのtagged baselineは7入力、24 source、実行binary、122 artifactのhashを固定し、当時の`93 tests / 7 suites`を含む証跡である。現行branchのmanifest契約は26 sourceへ更新済みだがformal rerun未実施であり、tagged baselineのrun IDや測定値を現行branchの合格証跡として扱わない。
...
2. Metal直接表示はpositive presentationと純粋なapp lifecycle reducerまで成立した。次はactual-screen parity、input-to-present p95、drop率、rapid supersede時のstale-frame、複数写真切替後の定常RSSだけを受入計測し、合格しない限り既定OFF・legacy表示を維持する
