...
現行sourceのv4 formal benchmarkは、process-fresh `358.478 ms`、warm high-quality `61.884 ms`、原寸JPEG `244.685 ms`が合格し、warm slider `60.120 ms`だけが50ms gateを超えた。manifest / source / binary / inputの開始・終了整合とeligible条件は満たすが、現行sourceのrunは1件だけなので安定性を証明しない。旧v4 / v3 runは履歴として残すが、現行反復へ混ぜない。[Google Benchmarkの反復・warm-up指針](https://google.github.io/benchmark/user_guide.html)を参考に全sampleを保存し、遅い値をoutlierとして削除・再試行しない。
